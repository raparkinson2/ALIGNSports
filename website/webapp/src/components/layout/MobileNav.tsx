...
export default function MobileNav() {
  ...
  // Show up to 4 feature items, then always show More
  const MAX_VISIBLE = 4;
  const visibleItems = featureItems.slice(0, MAX_VISIBLE);
  const isMoreActive = pathname.startsWith('/app/more') ||
    featureItems.slice(MAX_VISIBLE).some(i => pathname.startsWith(i.href));
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#0d1526] border-t border-white/[0.07]">
      <div className="flex">
        ...
        {/* More tab — always visible, links to /app/more */}
        <Link
          href="/app/more"
          className={cn(
            'flex-1 flex flex-col items-center justify-center gap-1 py-2.5 relative transition-colors',
            isMoreActive ? 'text-[#67e8f9]' : 'text-slate-500 hover:text-slate-300'
          )}
        >
